...
func (this *NetFlowV5) PayLoadToNetFlowV5(data []byte, host string) []NetFlowV5 {
	...
	if header.Version == 5 {
		...
		for i := 0; i < int(header.FlowRecords); i++ {
			...
			//beego.Error(fmt.Sprintf("srouce %s %s", src, curl.HttpGet("http://127.0.0.1:8080/check/"+src)))
			//beego.Error(fmt.Sprintf("srouce %s ", src),geoip.ParseStringIp(src))
			//dst := this.IntToIPv4Addr(record.Ipv4DstAddrInt).String()
			//beego.Error(fmt.Sprintf("srouce %s %s", dst, curl.HttpGet("http://127.0.0.1:8080/check/"+dst)))
			//beego.Error(fmt.Sprintf("srouce %s ", dst),geoip.ParseStringIp(dst))
